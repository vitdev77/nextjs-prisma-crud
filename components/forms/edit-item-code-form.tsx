...
import { editItemCode } from "@/actions/itemCode.actions";
import { getItems } from "@/actions/item.actions";
import { ItemCodeWithRelations } from "@/@types/prisma";
import { Check, ChevronsUpDown } from "lucide-react";
import { cn } from "@/lib/utils";

const editItemCodeSchema = z.object({
  itemCodeId: z.string().min(1, { message: "Item Code ID is required" }),
  code: z
    .string()
    .length(14, {
      message:
...
type EditItemCodeValues = z.infer<typeof editItemCodeSchema>;
...
export function EditItemCodeForm({ itemCode, _onSubmit }: Props) {
  ...
  const form = useForm<EditItemCodeValues>({
    resolver: zodResolver(editItemCodeSchema),
    defaultValues: {
      itemCodeId: String(itemCode.id),
      code: itemCode.code || "",
      itemId: String(itemCode.itemId),
      isUpdated: itemCode.isUpdated || true,
    },
  });

  const onSubmit = form.handleSubmit(async (data) => {
    const res = await editItemCode(data);
    if (res?.error) {
      toast.error(res.error);
    } else {
      toast.success("Item code data successfully updated");
      _onSubmit?.();
    }
  });
